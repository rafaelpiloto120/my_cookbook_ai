...
import {
  ...
  getDefaultServingForIngredient,
  MyDayMealIngredient,
...
function normalizeRecipeLineForRelevanceCheck(line: string) {
  return cleanRecipeIngredientLine(line)
    ...
    .replace(
      /^\s*(kg|g|mg|ml|l|lb|oz|tbsp|tbsps|tsp|tsps|tablespoons?|teaspoons?|cups?|colher(?:es)? de sopa|colher(?:es)? de chá|cucharadas?|cucharaditas?|cuill[eè]res? à soupe|cuill[eè]res? à caf[eé]|esslöffel|teelöffel|el|tl|clove|cloves|gousse|gousses|diente|dientes|dente|dentes|ramo|ramos|ramita|ramitas|branche|branches|folha|folhas|sheet|sheets|placa|placas|fatia|fatias|slice|slices|lata|latas|bo[iî]te|bo[iî]tes|dose|dosen|can|cans|pinch|pinches|pitada|pitadas|pizca|pizcas|pinc[eé]e|pinc[eé]es|prise|prisen|unidade|unidades|unit|units|un|und)\b\s*/i,
      ""
    )
...
function scoreRecipeIngredientLine(line: string) {
  ...
  if (
    /\b(cup|cups|tablespoon|tablespoons|teaspoon|teaspoons|tbsp|tsp|colher de sopa|colheres de sopa|colher de chá|colheres de chá|cucharada|cucharadita|cuillère à soupe|cuillère à café|esslöffel|teelöffel|el|tl)\b/.test(
      remainder
    )
  ) {
    score += 2;
  ...
  if (/\b(optional|opcional|facultatif|for garnish|for serving|to taste|a gosto|al gusto|au goût|nach geschmack|q\.?\s*b\.?)\b/.test(lower)) {
    score -= 6;
  }
...
function normalizeRecipeIngredientName(name: string) {
  const cleaned = name
    ...
    .replace(
      /^\s*(?:\d+(?:[.,]\d+)?(?:\s*-\s*\d+(?:[.,]\d+)?)?|\d+\/\d+|\d+\s+\d+\/\d+|½|¼|¾|⅓|⅔)\s*(kg|g|mg|ml|l|lb|oz)\s+/i,
      ""
    )
    .replace(
      /^\s*(?:\d+(?:[.,]\d+)?(?:\s*-\s*\d+(?:[.,]\d+)?)?|\d+\/\d+|\d+\s+\d+\/\d+|½|¼|¾|⅓|⅔)\s*(?:heaped|heaping|rounded|level|generous)?\s*(tbsp|tbsps|tsp|tsps|tablespoons?|teaspoons?|cups?|colher(?:es)? de sopa|colher(?:es)? de chá|cucharadas?|cucharaditas?|cuill[eè]res? à soupe|cuill[eè]res? à caf[eé]|esslöffel|teelöffel|el|tl)\s+/i,
      ""
    )
...
function isLowSignalLoggingIngredient(item: MyDayMealIngredient) {
...
function normalizePerServingRecipeIngredient(
  ...
  const serving = getDefaultServingForIngredient(String(item.name || ""));
  if (!serving) {
    return normalizedItem;
  }
...
export function parseRecipeIngredientLine(input: string): MyDayMealIngredient {
  ...
  const compactMetricMatch = trimmed.match(
    /^\s*((?:\d+(?:[.,]\d+)?|\d+(?:[.,]\d+)?\s*-\s*\d+(?:[.,]\d+)?|\d+\/\d+|\d+\s+\d+\/\d+|½|¼|¾|⅓|⅔))\s*(kg|g|mg|ml|l|lb|oz)\s+(.+)$/i
  );
  ...
  if (!quantityMatch) {
    return { name: trimmed, quantity: "1", unit: "serving" };
  }
...
export function buildRecipeMealLoggingRepresentationFromIngredients(
  ...
  const filteredSource =
    substantive.length >= 2 && ranked.length >= 4
      ? ranked
          .filter((entry) => !isLowSignalLoggingIngredient(entry.item))
          .map((entry) => entry.item)
      : ranked.map((entry) => entry.item);

  const prioritized = filteredSource.slice(0, limit);
...
export function buildRecipeMealLoggingRepresentation(
  ...
  const servings =
    typeof recipe.servings === "number" && Number.isFinite(recipe.servings) && recipe.servings > 0
      ? recipe.servings
      : 1;
  const ingredients = getRelevantRecipeIngredientLines(recipe, limit * 2)
    .map((line) => parseRecipeIngredientLine(line))
    .map((item) => ({ ...item }));
  return buildRecipeMealLoggingRepresentationFromIngredients(ingredients, limit, servings);
}
...
export function isRecipeMealLoggingRepresentationUsable(
  ...
  return unnamedCount === 0 && embeddedQuantityCount === 0;
}
...
export function estimateRecipeNutrition(recipe: SavedRecipe): RecipeNutritionEstimate {
  ...
  const servings =
    typeof recipe.servings === "number" && recipe.servings > 0 ? recipe.servings : 2;
  ...
  return {
    caloriesPerServing: Math.max(Math.round(totalEstimate.calories / servings), 80),
    proteinPerServing: Math.max(Math.round(totalEstimate.protein / servings), 4),
    carbsPerServing: Math.max(Math.round(totalEstimate.carbs / servings), 6),
    fatPerServing: Math.max(Math.round(totalEstimate.fat / servings), 3),
    servings,
  };
}
...
export async function resolveRecipeNutritionEstimate(
  ...
  if (recipe.nutritionInfo && !forceRepresentationRefresh) {
    const ingredients = getRelevantRecipeIngredientLines(recipe, 8).map((name) => ({
    ...
    return {
      ...
      ingredients,
      mealLoggingRepresentation: buildRecipeMealLoggingRepresentationFromIngredients(ingredients, 8),
      usedAiFallback: false,
    };
  }

  if (recipe.nutrition && !forceRepresentationRefresh) {
    const ingredients = getRelevantRecipeIngredientLines(recipe, 8).map((name) => ({
      name: String(name || "").trim(),
      quantity: "1",
      unit: "serving",
  ...
  try {
    const relevantIngredients = getRelevantRecipeIngredientLines(recipe, 8);
    const sourceLines = relevantIngredients.length > 0 ? relevantIngredients : [recipe.title].filter(Boolean);
    const parsedIngredients = sourceLines.map((line) => parseRecipeIngredientLine(line));
    const combinedIngredients = sourceLines.join(", ");
    ...
    const displayIngredients = parsedIngredients.map((ingredient, index) => {
    ...
    return {
      nutrition:
        savedNutrition ?? {
          caloriesPerServing: Math.max(Math.round(estimate.calories / servings), 80),
          proteinPerServing: Math.max(Math.round(estimate.protein / servings), 4),
          carbsPerServing: Math.max(Math.round(estimate.carbs / servings), 6),
          fatPerServing: Math.max(Math.round(estimate.fat / servings), 3),
          servings,
      },
      ingredients: displayIngredients,
      mealLoggingRepresentation: buildRecipeMealLoggingRepresentationFromIngredients(
        displayIngredients,
        8,
        servings
      ),
    ...
    const ingredients = getRelevantRecipeIngredientLines(recipe, 8).map((line) =>
      parseRecipeIngredientLine(line)
    );
    return {
      nutrition: fallback,
      ingredients,
...
export async function logRecipeMeal(
  ...
  return addRecipeMeal({
    ...
    ingredients:
      options?.ingredientsOverride ??
      (recipe.ingredients || []).slice(0, 6).map((name) => ({
        name,
        quantity: "100",
        unit: "g",
      })),
