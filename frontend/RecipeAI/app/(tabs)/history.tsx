...
export default function History() {
  ...
  const { bg, text, subText, card, border, cta, isDark, headerBg, headerText } = useThemeColors();
  const inlineAccentColor = isDark ? "#F2D6A3" : cta;
  const chipBg = "#FFFFFF";
      ...
      try {
        if (backendUrl && result.count > 0) {
          await recordEconomyEvent({
            ...
            metadata: { source: "file_import", count: result.count },
          });
        }
      } catch (rewardErr) {
        console.warn("[History] file import reward progress failed", rewardErr);
      }
  ...
  const [recipeActionTarget, setRecipeActionTarget] = useState<Recipe | null>(null);
    ...
    try {
      if (backendUrl) {
        await recordEconomyEvent({
          ...
          metadata: { cookbookId: newBook.id },
        });
      }
    } catch (rewardErr) {
      console.warn("[History] cookbook reward progress failed", rewardErr);
    }
  ...
  const startCookingRecipe = (recipe: Recipe) => {
    setRecipeActionTarget(null);
    router.push({
  ...
  const filteredRecipes = recipes.filter((r) => {
    ...
    const recipeTags = getNormalizedTags(r.tags);
    const matchesSearch =
      searchTerm.length === 0 ||
    ...
    const normalizedTags = recipeTags.map((t) => t.toLowerCase());
    const matchesTags =
    ...
    <View style={[styles.container, { backgroundColor: bg }]}>
      ...
      <View style={[styles.tabRow, { backgroundColor: card, borderColor: border }]}>
        <TouchableOpacity
          style={[
            styles.tab,
            { backgroundColor: activeTab === "all" ? cta : "transparent" },
          ]}
        ...
        <TouchableOpacity
          style={[
            styles.tab,
            { backgroundColor: activeTab === "cookbooks" ? cta : "transparent" },
          ]}
        ...
        >
          <Text
            style={[
              styles.tabText,
              { color: activeTab === "cookbooks" ? "#fff" : text },
            ]}
            numberOfLines={1}
          >
            {t("recipes.cookbooks")}
      ...
      {activeTab === "all" ? (
        <>
          ...
          <View style={[styles.searchRow, { backgroundColor: card, borderColor: border }]}>
            ...
            >
              <MaterialIcons name="filter-list" size={24} color={inlineAccentColor} />
              ...
              >
                <AppCard style={styles.recipeCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <RecipeTagRow tags={getNormalizedTags(item.tags)} />
                  </View>
                </AppCard>
              </TouchableOpacity>
            )}
        ...
        <>
          ...
          <FlatList
            ...
            keyExtractor={(item) => item.id}
            numColumns={2}
            contentContainerStyle={styles.cookbookListContent}
            renderItem={({ item }) => {
            ...
            ListEmptyComponent={
              <View style={{ alignItems: "center", marginTop: 40, paddingHorizontal: 16 }}>
                ...
                <TouchableOpacity onPress={() => setNewCookbookVisible(true)}>
                  <Text style={{ color: "#8A4B16", fontWeight: "600", textAlign: "center" }}>
      ...
      <NewRecipeOptionsModal
        ...
        onClose={() => setNewRecipeVisible(false)}
        onManualRecipe={() => {
      ...
      <Modal visible={importUrlVisible} transparent animationType="fade">
        ...
        >
          <View style={[styles.modalContent, { backgroundColor: card }]}>
            ...
            {importError ? (
              <Text style={{ color: "#8A4B16", marginBottom: 6, fontSize: 13 }}>
                {importError}
              </Text>
            ) : null}
            <TouchableOpacity
                ...
                try {
                  ...
                  if (isInstagram) {
                    ...
                    try {
                      ...
                      console.warn("[History] Instagram Reel reward progress failed", rewardErr);
                    }
                    setImportUrl("");
                    setImportUrlVisible(false);
                  ...
                  try {
                    if (backendUrl) {
                      await recordEconomyEvent({
                        ...
                        auth: firebaseAuth,
                        eventKey: "recipe_added",
                        metadata: { recipeId: recipe.id, source: isInstagram ? "instagram_reel" : "url_import" },
                      });
                    }
                  } catch (rewardErr) {
                    console.warn("[History] imported recipe reward progress failed", rewardErr);
                  }
                  ...
                  {["Easy", "Moderate", "Challenging"].map((d) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.filterOption,
                        {
                          backgroundColor: selectedDifficulties.includes(d) ? cta : chipBg,
                          borderColor: selectedDifficulties.includes(d) ? cta : border,
                        },
                      ]}
                      onPress={() =>
                    ...
                    >
                      <Text
                        style={{
                          color: selectedDifficulties.includes(d) ? "#fff" : chipText,
                        }}
                      >
            ...
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity onPress={() => setSortVisible(false)}>
                  <MaterialIcons name="close" size={24} color="#8F5E43" />
                </TouchableOpacity>
              </View>
              {sortOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected ? (
                      <MaterialIcons name="check" size={20} color="#8F5E43" />
                    ) : null}
                  </TouchableOpacity>
                );
              })}
          ...
          <View style={[styles.actionMenuContent, { backgroundColor: card }]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                const id = recipeActionTarget.id;
                setRecipeActionTarget(null);
                deleteRecipe(id);
              }}
            >
