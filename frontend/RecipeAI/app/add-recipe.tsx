...
useEffect(() => {
    const loadForEdit = async () => {
      try {
        if (params.editId) {
          ...
          if (found) {
            ...
            const normalizedDifficulty: "Easy" | "Moderate" | "Challenging" =
              allowedDifficulties.includes(found.difficulty) ? (found.difficulty as any) : "Easy";
            const normalizedCost: "Cheap" | "Medium" | "Expensive" =
              allowedCosts.includes(found.cost) ? (found.cost as any) : "Cheap";
            setEditingRecipe({ ...found, difficulty: normalizedDifficulty, cost: normalizedCost });
            setTitle(found.title);
            setCookingTime(String(found.cookingTime));
            ...
            setSteps((found.steps || []).join("\n"));
            setTags(found.tags || []);
            setImage(found.image || (found as any).imageUrl);
            // Normalize cookbooks to array of {id, name}
            let cookbookObjs: { id: string; name: string }[] = [];
            if (Array.isArray(found.cookbooks)) {
              cookbookObjs = found.cookbooks.map((cb: any) => {
                if (typeof cb === "string") {
                  // Try to find cookbook by ID in the cookbooks state (may not be loaded yet)
                  const match = cookbooks.find(c => c.id === cb);
                  return match ? { id: match.id, name: match.name } : { id: cb, name: "" };
        ...
        if (params.edit) {
          ...
          const allowedCosts = ["Cheap", "Medium", "Expensive"];
          const normalizedDifficulty: "Easy" | "Moderate" | "Challenging" =
            allowedDifficulties.includes(parsed.difficulty) ? (parsed.difficulty as any) : "Easy";
          const normalizedCost: "Cheap" | "Medium" | "Expensive" =
            allowedCosts.includes(parsed.cost) ? (parsed.cost as any) : "Cheap";
          setEditingRecipe({ ...parsed, difficulty: normalizedDifficulty, cost: normalizedCost });
          ...
          setTags(parsed.tags || []);
          setImage(parsed.image || (parsed as any).imageUrl);
          // Normalize cookbooks to array of {id, name}
          let cookbookObjs: { id: string; name: string }[] = [];
          if (Array.isArray(parsed.cookbooks)) {
            cookbookObjs = parsed.cookbooks.map((cb: any) => {
              if (typeof cb === "string") {
                const match = cookbooks.find(c => c.id === cb);
                return match ? { id: match.id, name: match.name } : { id: cb, name: "" };
              } else if (cb && typeof cb === "object" && cb.id && cb.name) {
    ...
    loadForEdit();
  }, [params.editId, params.edit, cookbooks]);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: bg }} edges={["left", "right", "bottom"]}>
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        ...
        <AppCard>
          ...
          {/* Save/Update button */}
          <AppButton
            label={editingRecipe ? t("recipes.update_recipe") : t("recipes.save_recipe")}
            onPress={saveRecipe}
            variant="primary"
            fullWidth
            style={[
